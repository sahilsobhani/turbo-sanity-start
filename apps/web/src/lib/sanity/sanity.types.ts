...
// Query: *[    _type == "blog" &&    $category in categories[]->slug.current &&    seoHideFromLists != true  ] | order(orderRank asc) {      _type,  _id,  title,  description,  "slug":slug.current,  richText,  orderRank,    image{    ...,    ...asset->{      "alt": coalesce(altText, originalFilename, "no-alt"),      "blurData": metadata.lqip,      "dominantColor": metadata.palette.dominant.background    },  },  publishedAt,    authors[0]->{    _id,    name,    position,      image{    ...,    ...asset->{      "alt": coalesce(altText, originalFilename, "no-alt"),      "blurData": metadata.lqip,      "dominantColor": metadata.palette.dominant.background    },  }  },    categories[]->{    _id,    title  },    pokemon {    id,    name,    sprite,    types,    base_experience,    height,    weight  },  }
